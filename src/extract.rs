...
fn try_collect_repeated_items_with_threshold(doc: &Document, min_words: usize) -> Option<String> {
    ...
    for container_sel in &container_selectors {
        ...
        for container_node in containers.nodes() {
            ...
            if let Some(group) = collect_sibling_group(&container, "article", 3, min_words) {
                let total: usize = group.iter().map(std::string::String::len).sum();
    ...
    if articles.length() >= 3 {
        ...
        if texts.len() >= 3 {
            let total: usize = texts.iter().map(std::string::String::len).sum();
            if total > best_total_len {
                best_total_len = total;
...
fn try_multi_candidate_merge(doc: &Document, _options: &Options) -> Option<String> {
    ...
    for tag in ["div", "section", "article", "main"] {
        for node in doc.select(tag).nodes() {
            ...
            while current.length() > 0 {
                if dom::tag_name(&current).is_some_and(|t| t == "body") {
                    break;
                }
    ...
    for c in &candidates {
        ...
        let overlaps = selected_texts.iter().any(|existing| {
            ...
            let _longer = c.text_len.max(existing.len());
            if shorter == 0 {
                return true;
            }
...
fn compute_extraction_quality_ml(
    ...
    if word_count > 50 {
        ...
        f[26] = f64::from(top) / (words.len() - 1).max(1) as f64;
    }
...
fn compute_extraction_quality_heuristic(
    ...
    let (expected_low, expected_mid) = match page_type {
        page_type::PageType::Article | page_type::PageType::Service => (1500, 5000),
            ...
            if extracted_from_content_node {
                if let Some(ref node) = content_node {
                    ...
                    for _ in 0..2 {
                        ...
                        if dom::tag_name(&ancestor).is_some_and(|t| t == "body" || t == "html") {
                            break;
                        }
                        let anc_text =
...
fn find_content_node_bottom_up(doc: &Document) -> Option<Selection<'_>> {
    let body = doc.select("body");
    if body.length() == 0 {
        return None;
    }
...
fn find_heuristic_content_node_with_options<'a>(
    ...
    for tag in ["div", "section", "article", "main"] {
        ...
        for node in elements.nodes() {
            ...
            let raw_text = dom::text_content(&el);
            let cleaned = clean_text(&raw_text);
            let text_len: i64 = i64::try_from(cleaned.len()).unwrap_or(i64::MAX);
            if text_len == 0 {
                continue;
            }
...
fn score_content_node(
    ...
    for node in p_elements.nodes() {
        ...
        let p_len: i64 = i64::try_from(p_clean.len()).unwrap_or(i64::MAX);
        if p_len >= 100 {
            substantive_p_count = substantive_p_count.saturating_add(1);
        }
    }

    let p_count: i64 = i64::try_from(el.select("p").length()).unwrap_or(i64::MAX);
    let _a_count: i64 = i64::try_from(el.select("a").length()).unwrap_or(i64::MAX);
    let h_count: i64 =
        i64::try_from(el.select("h1, h2, h3, h4, h5, h6").length()).unwrap_or(i64::MAX);
    ...
    for node in a_elements.nodes() {
        ...
        let a_len: i64 = i64::try_from(a_clean.len()).unwrap_or(i64::MAX);
            ...
            if let Some(tag_name) = node.node_name() {
                ...
                if is_heading {
                    ...
                    if let Some(itemprop) = dom::get_attribute(&heading_sel, "itemprop") {
                        if itemprop.eq_ignore_ascii_case("headline") {
                            skip_depths.push(depth);
                            continue;
                        }
                    }
